...
import { CreateTupleDrawer } from "./drawer"
import { Input } from "@/components/ui/input"
import { client } from "@/services/connect/client";
import { Tuple } from "./columns";
    ...
    <div className="h-full flex flex-col">
        <div className="flex items-center justify-between pb-4">
            ...
            <Button 
                ...
                onClick={async () => {
                    const selected = table.getFilteredSelectedRowModel().rows as { original: Tuple }[]
                    try {
                        const out = await client.deleteTuples({
                            mode:{
                                case: "tuples",
                                value: {
                                    tuples:
                                    selected.map((r) => ({
                                        sbjNs: r.original.sbjNs,
                                        sbjId: r.original.sbjId,
                                        rel: r.original.relation,
                                        objNs: r.original.objNs,
                                        objId: r.original.objId,
                                    }))
                                }
                            }
                        });
                        toast.success("Tuple delete successfully!");
                    } catch (e) {
